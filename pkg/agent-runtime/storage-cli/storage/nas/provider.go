...
// Package nas provides the Alibaba Cloud NAS CSI storage runtime adapter.
package nas
...
func (p *provider) Validate(req csi.NodePublishVolumeRequest) error {
	if strings.TrimSpace(req.VolumeId) == "" {
		return fmt.Errorf("NAS CSI volume ID is required")
	}
	if strings.TrimSpace(req.TargetPath) == "" {
		return fmt.Errorf("NAS CSI target path is required")
	}
	if req.VolumeCapability == nil {
		return fmt.Errorf("NAS CSI volume capability is required")
	}
	if strings.TrimSpace(req.VolumeContext["server"]) == "" {
		return fmt.Errorf("NAS CSI volume context server is required")
	}
	if strings.TrimSpace(req.VolumeContext["path"]) == "" {
		return fmt.Errorf("NAS CSI volume context path is required")
	}
	return nil
}
