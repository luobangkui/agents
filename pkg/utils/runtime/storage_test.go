...
func TestStorageUnmountSendsDeleteWithOriginalRequest(t *testing.T) {
	type observed struct {
		method string
		body   []byte
	}
	...
	_, sbx := newMountTestServer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		seen <- observed{method: r.Method, body: raw}
		writeMountResponse(t, w, http.StatusOK, DeleteMountResponse{Success: true, MountPath: "/m"})
	})
	req := testMountRequest("ossplugin.csi.alibabacloud.com")

	unmountAPI, ok := NewRuntime(sbx, WithRetry(fastBackoff)).Storage().(StorageUnmountAPI)
	require.True(t, ok)
	_, err := unmountAPI.Unmount(context.Background(), req)

	require.NoError(t, err)
	got := <-seen
	require.Equal(t, http.MethodDelete, got.method)
	var received DeleteMountRequest
	require.NoError(t, json.Unmarshal(got.body, &received))
	require.Equal(t, req.Driver, received.Driver)
	require.True(t, protoEqual(req.PublishRequest, received.PublishRequest))
}

func TestStorageUnmountRejectsMissingIdentity(t *testing.T) {
	_, sbx := newMountTestServer(t, func(http.ResponseWriter, *http.Request) {
		t.Fatal("request without CSI identity must not be dispatched")
	})

	unmountAPI, ok := NewRuntime(sbx, WithRetry(fastBackoff)).Storage().(StorageUnmountAPI)
	require.True(t, ok)
	_, err := unmountAPI.Unmount(
		context.Background(), DeleteMountRequest{Driver: "oss"},
	)

	require.ErrorContains(t, err, "publish request is required for unmount")
}
